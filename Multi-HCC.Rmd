...
df = cohort %>% 
  ...
  mutate(
    ...
    # INIT_DATE (part of exception data) = start date of listing for non-HCC
    # BEGIN_DATE = start date of listing for HCC
    # END_DATE = earliest of dates of removal from waiting list, transplant, 
    # death, or time copy of data created
    fu_days = as.numeric(END_DATE - BEGIN_INIT_DATE), 
    # 90 days
...
ragg::agg_tiff("img/log_afp_spline.tiff", 
...
# Coefficient summary
cbind("exp(coef)" = exp(coef(fit_coxph_6months[[my_model]])), 
      as.data.frame(exp(confint(fit_coxph_6months[[my_model]]))) %>%
        rename_with(~paste("exp(coef)", .x))) %>% 
...
coxph_HCC_slope = coef(fit_coxph_HCC_nonHCC_6months)["HCC:preds6months"]
coxph_HCC = coef(fit_coxph_HCC_nonHCC_6months)["HCC"]
coxph_nonHCC = coef(fit_coxph_HCC_nonHCC_6months)["nonHCC_MELD"]
...
my_intercept_6months = (coxph_HCC / coxph_nonHCC) -
  (coxph_HCC_slope / coxph_nonHCC) * coxph_shift
# Coefficients for HCC patients
my_coefs_6months = (coxph_HCC_slope / coxph_nonHCC) * 
...
ragg::agg_tiff("img/ph_schoenfeld.tiff", 
...
Mean number of MELD-equivalent points to add for each MELD 3.0 score. Error bars indicate the +/-1 standard error, and those with MELD 3.0 above 30 are grouped together. 

```{r, fig.height = 8}
# Scatterplot of points to add against MELD
multi_HCC_mean_df = test_df %>% 
...
p1 = test_df %>% 
  ...
  geom_point(data = multi_HCC_mean_df[multi_HCC_mean_df$Cohort == "HCC", ], 
             aes(colour = Cohort)) + 
  geom_errorbar(aes(ymin = mu - sd, ymax = mu + sd), width = 0,
                 position = position_dodge(0.1)) + 
  scale_x_continuous(breaks = 6:30, limits = c(5.9, 30.1), 
                     labels = c(6:29, "30+")) + 
  scale_y_continuous(breaks = scales::breaks_pretty(), limits = c(-4, 7)) + 
  # geom_vline(xintercept = c(6, 10, 20, 30), color = "grey") +
  xlab("MELD 3.0") + ylab("Mean number of MELD-equivalent points to add")
...
ragg::agg_tiff("img/pts_to_add.tiff", 
...
ragg::agg_tiff("img/hist_score.tiff", 
               width = 5, height = 5, units = "in", res = 300)
test_df %>% 
  filter(HCC == TRUE) %>% 
...
ragg::agg_tiff("img/nonhcc_vs_hcc.tiff", 
...
Mosaic plot showing the distribution of Multi-HCC quartiles for HCC patients with each MELD 3.0 scores (widths of bars correspond to number of patients with that MELD 3.0 score). MELD 3.0 scores above 30 are collapsed. 
...
mosaic_df = test_df %>% 
  ...
  mutate(quant = cut(multi_HCC, breaks = quantile(multi_HCC)), 
         MELD3 = case_when(
           ...
           TRUE ~ MELD3)) %>% 
  drop_na(quant) %>% 
  group_by(MELD3, quant) %>% 
  summarize(n = n()) %>%
  mutate(N = sum(n, na.rm = TRUE)) %>% 
  complete(quant, fill = list(n = 0)) %>%
  ungroup()

ragg::agg_tiff("img/meld3_quartiles.tiff", 
               width = 7, height = 5, units = "in", res = 300)
mosaic_df %>% 
  ggplot() +
  geom_mosaic(aes(x = product(MELD3), weight = n, fill = quant), 
              offset = 0, color = "black") + 
  scale_fill_discrete(name = "Multi-HCC quartiles", 
                      labels = paste("Quartile", quartile_label)) + 
  scale_x_productlist(
    breaks = (cumsum(unique(na.omit(mosaic_df$N))) - 
                unique(na.omit(mosaic_df$N))/2) / 
      sum(unique(na.omit(mosaic_df$N))), 
    labels = c(6:18, "", 20, "", 22, rep("", 7), "30+"), 
    expand = c(0, 0)) + 
  scale_y_continuous(labels = scales::percent, expand = c(0, 0)) + 
  xlab("MELD 3.0") + ylab("Percent") + 
  theme(legend.position = "bottom")
invisible(dev.off())
...
Scatterplot of Multi-HCC against MELD 3.0. The point sizes correspond with the number of HCC patients with the given scores. Scores above 30 are collapsed. 

```{r, fig.width = 4.5}
ragg::agg_tiff("img/multihcc_vs_meld3.tiff", 
...
"Table 1" of HCC cohort characteristics, by training and test set. 
...
Overall waiting list dropout at 90 days, 6 months, and 1 year among test set patients with HCC, estimated using Kaplan-Meier. 
...
Observed risk of waiting list dropout at 6 months for each Multi-HCC quartile in the test set, estimated using Kaplan-Meier. 
...
surv_5year = survfit(Surv(time_tx_px, fail5year) ~ Quartile, 
                     data = HCC_test_5year_df)
survdiff_5year = survdiff(Surv(time_tx_px, fail5year) ~ Quartile, 
                          data = HCC_test_5year_df)

ragg::agg_tiff("img/km_5year.tiff", 
               width = 7, height = 5, units = "in", res = 300)
ggsurvplot(surv_5year, data = HCC_test_5year_df, 
           xlab = "Days", ylab = "Post-survival transplant probability", 
           legend.title = "", legend.labs = paste("Quartile", quartile_label),
           risk.table = FALSE, censor = FALSE, 
           pval = round(survdiff_5year$pvalue, 2), 
...
5-year post-transplant survival, by quartile, estimated using Kaplan-Meier. 

```{r}
as.data.frame(tibble(
  Quartile = c("1 (Low)", "2", "3", "4 (High)"), 
  "5-year post-transplant survival" = 
    paste0(round(100 * summary(surv_5year, times = 5*365)$surv, 1), "%")
)) %>% `rownames<-`(NULL) %>% 
  kbl() %>%
  kable_styling()
```

5-year post-transplant survival among test patients with Multi-HCC 16-19, 20-23, and above 24, estimated using Kaplan-Meier. 
...
We use a bootstrap routine to obtain 95% confidence intervals for each 6-month model's test concordance (Harrell's and Uno's) and integrated Brier score (multi-HCC only).  
...
HCC_test_20190514_df = HCC_test_df %>% 
  filter(BEGIN_INIT_DATE > as.Date("2019-05-14"))
```
...
# When parallelized to 8 cores, takes ~2 minutes
boot_diagnostics = foreach::foreach(
  ...
  # Uno's C
  concordance_uno_est = apply(boot_scores_test, 2, function(pred) {
    if(all(is.na(pred))) {
      return(NA)
    }
    concordance(Surv(fu_days_6months, removed_status_6months) ~ pred, 
                data.frame(pred = pred,
                           boot_HCC_test_df %>% 
                             dplyr::select(fu_days_6months, 
                                           removed_status_6months)), 
                reverse = TRUE, timewt = "n/G2")$concordance
    })
  ...
  return(list(concordance_est = concordance_est, 
              concordance_uno_est = concordance_uno_est, 
              concordance_est_20190514 = concordance_est_20190514, 
              concordance_uno_est_20190514 = concordance_uno_est_20190514, 
              brier_est = brier_est))
}
...
We report the test concordance for each model with a 95% bootstrap CI and p-values for a significant difference from Multi-HCC. All p-values reported have been adjusted using the Holm correction. Both Harrell's C-statistic and Uno's C-statistic are shown. 
...
# Uno's C
# Point estimates for concordance
concordance_uno_est = apply(scores_test, 2, function(pred) {
  concordance(Surv(fu_days_6months, removed_status_6months) ~ pred, 
              data.frame(pred = pred,
                         HCC_test_df %>% 
                           dplyr::select(fu_days_6months, 
...
# Concordance 2.5%
concordance_uno_lo = 
  apply(sapply(boot_diagnostics, function(x){x$concordance_uno_est}), 
        1, quantile, 0.025, na.rm = TRUE)
# Concordance 97.5%
concordance_uno_hi = 
  apply(sapply(boot_diagnostics, function(x){x$concordance_uno_est}), 
...
concordance_tab = data.frame(
  harrell = paste0(round(concordance_est, 2), " (", 
                   round(concordance_lo, 2), ", " ,
                   round(concordance_hi, 2), ")"), 
  uno = paste0(round(concordance_uno_est, 2), " (", 
               round(concordance_uno_lo, 2), ", " ,
               round(concordance_uno_hi, 2), ")")
) %>% setNames(c("Harrell's C-statistic (95% CI)", 
                 "Uno's C-statistic (95% CI)")) %>% 
  `rownames<-`(names(concordance_est))
```

```{r, eval = FALSE}
# Compare Multi-HCC to other models
# Harrell's C
...
# Uno's C
# 2.5 hours to run
pvals_uno_6months = foreach::foreach(
  i = 1:(ncol(scores_test)-1), .packages = c("tidyverse", "survC1"), 
  .combine = c
  ) %dopar% {
    # Calculate difference in Uno's C-statistic and the standard error, 
    # using a perturbation-resampling method
    my_Delta = Inf.Cval.Delta(
      mydata = data.frame(time = HCC_test_df$fu_days_6months, 
                          event = HCC_test_df$removed_status_6months), 
      covs0 = HCC_test_df$multi_HCC, 
      covs1 = scores_test[,-3][,i], 
      tau = 180, 
      itr = 1000, 
      seed = 1)
    
    # Get p-value using normal approximation
    return(pnorm(my_Delta["Delta", "Est"], 
                 mean = 0, sd = my_Delta["Delta", "SE"]))
  }
names(pvals_uno_6months) = names(scores_test[,-3])

save(pvals_6months, pvals_uno_6months, file = "pvals_6months.rData")
```

```{r, echo = FALSE}
load("pvals_6months.rData")
```

```{r}
# Adjust all p-values (Holm's adjustment)
pvals_adj = p.adjust(pvals_6months)
pvals_uno_adj = p.adjust(pvals_uno_6months)

# Add p=values to table of C-statistics
data.frame(
  harrell = concordance_tab[c("MELD 3.0 + log(AFP) + tumor burden (Multi-HCC)", 
                    names(pvals_6months)), 
                    "Harrell's C-statistic (95% CI)", drop = FALSE], 
  harrell_p = c("", ifelse(pvals_adj[names(pvals_6months)] < 0.01, 
                           "<0.01", 
                           round(pvals_adj[names(pvals_6months)], 2))), 
  uno = concordance_tab[c("MELD 3.0 + log(AFP) + tumor burden (Multi-HCC)", 
                          names(pvals_uno_6months)), 
                        "Uno's C-statistic (95% CI)", drop = FALSE], 
  uno_p = c("", ifelse(pvals_uno_adj[names(pvals_uno_6months)] < 0.01, 
                       "<0.01", 
                       round(pvals_uno_adj[names(pvals_uno_6months)], 2)))) %>% 
  setNames(c("Harrell's C-statistic (95% CI)", 
             "Adjusted p-value", 
             "Uno's C-statistic (95% CI)", 
             "Adjusted p-value")) %>% 
...
As a sensitivity check, we calculate the Multi-HCC C-statistics when restricting to patients who were waitlisted after May 14, 2019. 
...
concordance_uno_est_20190514 = 
  concordance(Surv(fu_days_6months, removed_status_6months) ~ multi_HCC, 
              ...
              reverse = TRUE, timewt = "n/G2")$concordance
# Concordance 2.5%
concordance_uno_lo_20190514 = quantile(
  sapply(boot_diagnostics, function(x){x$concordance_uno_est_20190514}), 
  0.025, na.rm = TRUE)
# Concordance 97.5%
concordance_uno_hi_20190514 = quantile(
  sapply(boot_diagnostics, function(x){x$concordance_uno_est_20190514}), 
  0.975, na.rm = TRUE)

# Concordance statistic with 95% CI
data.frame(
  harrell = paste0(round(concordance_est_20190514, 2), " (", 
                   round(concordance_lo_20190514, 2), ", " ,
                   round(concordance_hi_20190514, 2), ")"), 
  uno = paste0(round(concordance_uno_est_20190514, 2), " (", 
               round(concordance_uno_lo_20190514, 2), ", " ,
               round(concordance_uno_hi_20190514, 2), ")")
) %>% setNames(c("Harrell's C-statistic (95% CI)", 
                 "Uno's C-statistic (95% CI)")) %>% 
  kbl() %>%
  kable_styling()
...
# Kaplan-Meier plot
p_km_6months = ggsurvplot(
  survfit(Surv(fu_days_6months, removed_status_6months) ~ 
...
ragg::agg_tiff("img/calib_km.tiff", 
               width = 4.5, height = 5, units = "in", res = 300)
p_km_6months$plot +
  geom_point(
