...
# Coefficient summary
cbind(summary(fit_coxph_6months[[my_model]])$coefficients, 
...
ggsurvplot(survfit(Surv(fu_days_90days, removed_status_90days) ~ Cohort, 
           ...
           xlab = "Days", ylab = "Proportion not removed from waitlist", 
...
concordance_tab = data.frame(
  harrell = paste0(round(concordance_est, 3), " (", 
                   round(concordance_lo, 3), ", " ,
                   round(concordance_hi, 3), ")"), 
  uno = paste0(round(concordance_uno_est, 3), " (", 
               round(concordance_uno_lo, 3), ", " ,
               round(concordance_uno_hi, 3), ")")
...
data.frame(
  ...
  harrell_p = c("", ifelse(pvals_adj[names(pvals_6months)] < 0.001, 
                           "<0.001", 
                           round(pvals_adj[names(pvals_6months)], 4))), 
  uno = concordance_tab[c("MELD 3.0 + log(AFP) + tumor burden (Multi-HCC)", 
                          names(pvals_uno_6months)), 
                        "Uno's C-statistic (95% CI)", drop = FALSE], 
  uno_p = c("", ifelse(pvals_uno_adj[names(pvals_uno_6months)] < 0.001, 
                       "<0.001", 
                       round(pvals_uno_adj[names(pvals_uno_6months)], 4)))) %>% 
  setNames(c("Harrell's C-statistic (95% CI)", 
             "Adjusted p-value", 
             "Uno's C-statistic (95% CI)", 
             "Adjusted p-value")) %>% 
...
data.frame(
  harrell = paste0(round(concordance_est_20190514, 3), " (", 
                   round(concordance_lo_20190514, 3), ", " ,
                   round(concordance_hi_20190514, 3), ")"), 
  uno = paste0(round(concordance_uno_est_20190514, 3), " (", 
               round(concordance_uno_lo_20190514, 3), ", " ,
               round(concordance_uno_hi_20190514, 3), ")")
) %>% setNames(c("Harrell's C-statistic (95% CI)", 
                 "Uno's C-statistic (95% CI)")) %>% 
  kbl() %>%
  kable_styling()
